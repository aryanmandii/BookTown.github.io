...
dropdownItems.forEach((el) => {
    el.addEventListener("click", (e) => {
        ...
        if (sortElement.innerHTML == " : Cheapest to most expensive") {
            ...
            productMaker(date);
        } else if (sortElement.innerHTML == " : Best Of Bookhala") {
...
categorieSelector.forEach((el) => {
    el.addEventListener("click", (e) => {
        ...
        if (e.target.innerHTML == "Self-Development") {
            ...
            productMaker(getBooks);
        } else if (e.target.innerHTML == "Best Of BookShala") {
            const rating = books().sort(function(a, b) {
                let rateA = a.rating,
                    rateB = b.rating;
                return rateB - rateA;
    ...
    if (el.target.innerText == "Add to Cart ") {
        let data = {
            ...
            name: el.target.parentElement.parentElement.children[1].children[0]
                .innerText,
            price: el.target.parentElement.parentElement.children[1].children[2].children[1].innerText.slice(
                1
            ),
            value: 1,
            lastprice: (() => {
